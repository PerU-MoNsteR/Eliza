...
@borg.on(sudo_cmd(pattern="(en|re|li)ai$", allow_sudo=True))
async def lydia_disable_enable(event):
    ...
    if Config.LYDIA_API_KEY is None:
        ...
        return
    webevent = await edit_or_reply(event, "`.....`")
    ...
    if event.reply_to_msg_id is not None:
        ...
        if input_str == "en":
            ...
            await webevent.edit(f"Hello")
        elif input_str == "re":
            remove_s(user_id, chat_id)
            await webevent.edit(f"__[signal lost](tg://user?id={user_id})__")
        else:
            lsts = get_all_s()
            if len(lsts) > 0:
            ...
            if len(output_str) > Config.MAX_MESSAGE_SIZE_LIMIT:
                ...
                await webevent.edit(output_str)
    else:
        if input_str == "li":
            lsts = get_all_s()
            if len(lsts) > 0:
