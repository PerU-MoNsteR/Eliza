...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return repo.__del__()
        await event.edit(
            "`[HEROKU]`" "\n`Userbot build in progress on your Eliza, please wait...`"
        )
        ups_rem.fetch(ac_br)
        ...
        await event.edit("`Successfully deployed your Eliza!\n" "Restarting, please wait...`")
    else:
        await event.edit(
...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n`Eliza has`  has no new updates  `with`  "
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            'do "[`.update now`] or [`.update deploy`]" to update.Check `.info updater2` for details'
        )
