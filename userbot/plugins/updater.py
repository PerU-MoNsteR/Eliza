...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return repo.__del__()
        await event.edit(
            "`Userbot build in progress, please wait until the process finishes it usually takes 4 minutes .`"
        )
        ups_rem.fetch(ac_br)
...
async def upstream(event):
    ...
    if conf == "deploy":
        await event.edit("`Deploying Eliza, please wait....`")
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            'do "[`.update now`] or [`.update deploy`]" to update for force update`.help updater` '
        )
