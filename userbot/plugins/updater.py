...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return repo.__del__()
        await event.edit(
            "`[HEROKU]`" "\n`Userbot updates are in progress, please wait...`"
        )
        ups_rem.fetch(ac_br)
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit(
        "`Successfully Updated!\n" "Eliza UserBot is restarting... Wait for a minute!`"
    )
...
async def upstream(event):
    ...
    if conf == "deploy":
        await event.edit("`Deploying Eliza userbot, please wait....`")
        await deploy(event, repo, ups_rem, ac_br, txt)
        return
    if changelog == "" and not force_update:
        await event.edit(
            "\n`Eliza USERBOT is`  ON TRACK WITH TODAY'S UPDATE  `with`  "
